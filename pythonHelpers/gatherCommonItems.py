...
class FetchThread(threading.Thread):
	...
	def run(self):
		for (name, url, exp, price, count) in self.list:
			page = requests.get(url)
			source = page.content
			self.results += self.gatherDetails(name, exp, price, count, BeautifulSoup(source, "html.parser"))
			self.results += "},\n"

	def gatherDetails(self, name, exp, price, count, soup):
		results = '{{\n"name":"{}",\n"expansion":"{}",\n"price":"{}",\n"count":{},\n'.format(cleanUp(name),
																							 exp,
																							 cleanUp(price),
																							 cleanUp(count))
...
def getItemList(soup, base_url):
	start = soup.find(id="List_of_Common_Items").parent
...
page = requests.get(base_url + "/Common_item")
page.encoding = "ISO-8859-1"
source_soup = BeautifulSoup(page.text, "html.parser")
common_items = getItemList(source_soup, base_url)

threads = []
numThreads = 3
threadLength = len(common_items)//numThreads
start = 0
for i in range(0,numThreads):
	thread = FetchThread(common_items[start:start+threadLength])
	thread.start()
	threads.append(thread)
	start += threadLength
thread = FetchThread(common_items[start:])
thread.start()
threads.append(thread)
...
with open("common_items.json", "w", encoding="utf8") as outfile:
