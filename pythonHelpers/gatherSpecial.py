...
class FetchThread(threading.Thread):
	...
	def run(self):
		for (name, url, type, exp) in self.list:
			page = requests.get(url)
			source = page.content
			self.results += self.gatherDetails(name, type, exp, BeautifulSoup(source, "html.parser"))
			self.results += "},\n"

	def gatherDetails(self, name, type, exp, soup):
		results = '{{\n"name":"{}",\n"expansion":"{}",\n"type":"{}",\n'.format(cleanUp(name),
																			   cleanUp(exp.split(':')[0]),
																			   cleanUp(type))
...
def getItemList(soup, base_url):
	start = soup.find(id="List_of_Special_Cards").parent
...
page = requests.get(base_url + "/Special")
...
with open("special.json", "w", encoding="utf8") as outfile:
