...
class FetchThread(threading.Thread):
	...
	def run(self):
		for (name, url, count, match, exp) in self.list:
			page = requests.get(url)
			source = page.content
			self.results += self.gatherDetails(name, count, match, exp, BeautifulSoup(source, "html.parser"))
			self.results += "},\n"

	def gatherDetails(self, name, count, match, exp, soup):
		results = '{{\n"name":"{}",\n"expansion":"{}",\n"count":"{}",\n"matching_injury":"{}",\n'.format(cleanUp(name),
																			   cleanUp(exp.split(':')[0]),
																			   cleanUp(count),
																			   cleanUp(match))
...
def getItemList(soup, base_url):
	start = soup.find(id="List_of_madnesses").parent
...
page = requests.get(base_url + "/Madness")
...
with open("madness.json", "w", encoding="utf8") as outfile:
