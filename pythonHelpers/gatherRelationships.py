...
class FetchThread(threading.Thread):
	...
	def gatherDetails(self, name, exp, soup):
		...
		try:
			...
			temp = []
			while runon:
				temp.append(cleanUp(runon.text))
...
def getItemList(soup, base_url):
	start = soup.find(id="List_of_Relationships").parent
...
page = requests.get(base_url + "/Relationship")
...
with open("relationships.json", "w", encoding="utf8") as outfile:
